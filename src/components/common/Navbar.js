const Navbar = ({ isMobileNavOpen, showMobileNav }) => {
  return (
    <div className="relative w-full flex flex-wrap justify-between items-center pt-4 px-10 lg:px-20 lg:pt-7 bg-black">
      <div className="flex items-center w-auto space-x-16 h-9 text-white">
        <svg
          width="100%"
          height="auto"
          viewBox="0 0 945 202"
        ...
        <p
          className={`text-md pt-2 font-extralight hidden lg:block ${
            isMobileNavOpen
              ? "transition-all delay-300 opacity-0 -translate-y-full ease-out"
      ...
      <div className="flex items-center space-x-16">
        <p
          className={`text-md font-extralight hidden lg:block pt-2 text-white ${
            isMobileNavOpen
              ? "transition-all delay-300 opacity-0 -translate-y-full ease-out"
              : "transition-opacity opacity-100 duration-700 translate-x-0"
          }`}
        ...
        <div
          className="flex flex-col items-end space-y-2 group pt-2 cursor-pointer"
          onClick={() => showMobileNav(!isMobileNavOpen)}
        >
          <div
            className={` bg-white w-7 h-0.5 group-hover:w-9 transition ease transform duration-300 ${
              isMobileNavOpen
                ? "rotate-45 translate-y-3  group-hover:opacity-100 w-9"
                : " group-hover:opacity-100"
            }`}
            // style="background-color: rgb(243, 243, 243); width: 28px; height: 2px;"
          ></div>
          <div
            className={` bg-white w-9 h-0.5 transition ease transform duration-300 ${
              isMobileNavOpen ? "opacity-0" : " group-hover:opacity-100"
            }`}
            // style="background-color: rgb(243, 243, 243);"
          ></div>
          <div
            className={` bg-white w-8 h-0.5 group-hover:w-9 transition ease transform duration-300 ${
              isMobileNavOpen
                ? "-rotate-45 -translate-y-3  group-hover:opacity-100 w-9"
                : " group-hover:opacity-100"
            }`}
            // style="background-color: rgb(243, 243, 243); width: 31.5px; height: 2px;"
          ></div>
        </div>
      </div>
    </div>
